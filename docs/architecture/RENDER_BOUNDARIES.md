...
| Overlay composite | `composite_overlays_on_base_clip()` | `render_engine.py` (Phase 4E.4: → `render/overlay_compositor.py`) | subtitle ASS, title drawtext, text_layer drawtext, fps=, -c:a copy | setpts, atempo, crop, scale, color, effect, BGM, loudnorm |
