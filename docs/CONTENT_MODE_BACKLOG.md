...
| CM-3 | ✅ DONE | 2026-07-07 | `run_content` khi `resume_mode` đọc lại `content_plan_json` (reader `get_content_plan` đã có) và ưu tiên nó trên override/AI → resume dùng đúng plan/index khớp scene trên disk (đóng G2+G4). CRITICAL — Render Edit Protocol đầy đủ: baseline 2572 → edit tối thiểu (chỉ block plan-resolution) → full pytest. Test `test_run_content_resume_prefers_persisted_plan` (e2e). Chưa commit. |
| CM-4 | ✅ DONE | 2026-07-07 | **Thiết kế hiệu chỉnh** (điều tra: prune đã skip active; `interrupted` là gap thật): thay vì relocate `scenes_dir`, `prune_render_temp_dirs` giữ temp dir job `interrupted`/`paused` trong `CONTENT_RESUME_KEEP_HOURS` (72h, theo mtime). MEDIUM tier (`services/maintenance.py`) — **không còn phụ thuộc CM-5**. Test `test_content_resume_keep.py` (4). Regression maintenance 31 passed. Chưa commit. |
| CM-5 | ❌ DROPPED | 2026-07-07 | Bỏ hẳn. Điều tra: `sid` KHÔNG cần — (a) resume do CM-3 (persisted plan) lo, index khớp; (b) temp file index-based ổn định vì `buildPayload` reindex dày trước submit; (c) CM-11 drag dùng drop rời rạc → `key={i}` đủ. Không có use case còn lại → không làm. |
| CM-6 | ✅ DONE | 2026-07-07 | Full 6-module decomposition. `run_content` **809 → 326 dòng**. Trích `stages/content/`: `context.py` (ContentRenderContext + stable_seed/safe_filename), `scene_stage.py` (render_one_scene — gỡ God-closure 137 dòng), `provider_stage.py`, `plan_stage.py`, `assembly_stage.py`, `finalize_stage.py`. CRITICAL — Render Edit Protocol từng module (A/B/C/D+E), mỗi bước full pytest **2585 = baseline** (bit-identical); e2e ffmpeg render thật. Test patch repoint sang home mới. 5 commit. |
| CM-7 | ✅ DONE (experimental) | 2026-07-07 | **Tối ưu: 0 prompt mới** (thay vì 5-bước/2-prompt). `CONTENT_PLAN_MODE=quality` thêm 1 pass narration-refine trong content_director, **tái dùng** `build_content_narration_refine_prompt` + `parse_episode_narration_response` qua call_fn. Default `fast` = không đổi (zero regression). HIGH tier, chỉ chạm `content_director.py`. Test `test_content_quality_mode.py` (4). **ai_eval ĐÃ CHẠY (2026-07-07):** Δ−0.111±0.091 inconclusive, QUAL 0/3, faithfulness−0.333 → **KHÔNG uplift → giữ default `fast`** (ghi tại ai_eval/DECISIONS.md; harness `ab_content_quality.py`). |
