...
> **Status: IMPLEMENTED qua R7** (R1–R6 + two-pass story intelligence). Spec
> gốc viết 2026-06-29; các mục R6 (§13) và R7 (§14) là addendum append-only.
...
## 14. R7 — Two-pass Story Intelligence (2026-06-30)

**Vấn đề R1–R6:** recap chỉ là **1 LLM call** đọc transcript → RecapPlan, với
`thinking_budget=0` (không suy luận) và `story_summary` mà AI sinh ra bị parser
**vứt đi**. Tức là "tóm tắt transcript", chưa phải "hiểu truyện".

**Thay đổi cốt lõi:** tách thành **2 pass**, giữ nguyên contract `select_recap_plan`
(pipeline + dispatch + mode `clips` không đổi). Master switch `RECAP_TWO_PASS`
(mặc định ON; `=0` → về single-pass R6 ngay, không revert code).

```
Pass 1  Story Understanding (reasoning ON)  → StoryModel
            {summary, characters[], beats[], climax, ending}
Pass 2  Editorial Planning (điều kiện hóa BẰNG StoryModel) → RecapPlan
Pass 1 None  → fallback single-pass (Sacred #3, không bao giờ chặn render)
```

**Data model** (`domain/recap_plan.py`, `schema_version=3`):
```
RecapPlan{ story: StoryModel, episodes[]… }
StoryModel{ summary, characters[], beats[], climax, ending }
```
- `RecapPlan.story_summary` đổi từ **field → @property** = `story.summary`
  (back-compat: 4 test R-v1 vẫn xanh).
- `from_json` đọc **cả 2 dạng**: nested `"story"` (R7) hoặc flat `"story_summary"`
  (blob v1) → bọc vào `StoryModel(summary=...)`. Blob cũ replay bit-identical
  (Sacred #2/#3). `story_model_from_dict` phòng thủ, không raise.

**AI layer** (cả 3 provider):
- `recap_prompts.py`: `build_story_model_prompt` (pass-1); `build_recap_prompt(..., story_model=)`
  inject khối Story Model **format-safe** (truyền qua `str.format` như VALUE +
  neutralise `{`/`}` → không leak brace, theo cảnh báo CLAUDE.md cho prompts.py).
  Cả 2 template **transcript-first** để bật caching.
- `recap_parser.py`: `parse_story_model_response` + salvage JSON bị cắt (Sacred #3).
- `__init__.py`: `select_story_model` dispatcher + `_get_story_impl` (defensive)
  orchestrate two-pass **một chỗ** — provider chỉ thêm primitive `select_story_model`,
  không nhân 3 logic.
- gemini: `_call_gemini_story` thinking 8192 · openai: `reasoning_effort` chỉ cho
  o-series/gpt-5 · claude: extended-thinking opt-in (`CLAUDE_STORY_THINKING_BUDGET`).

**Caching (Stage C):**
- gemini (default) + openai: **implicit prefix caching** tự động nhờ transcript-first
  (0 dòng code thêm).
- claude: `cache_control: ephemeral` trên user-block recap/story
  (`CLAUDE_RECAP_CACHE=0` để tắt). **Chỉ recap/story path — clips path không đụng.**

**Pipeline** (`recap_pipeline.py`): StoryModel được surface vào event
`recap.plan.ready` (`story_model{summary,characters,beats,climax,ending}` +
`story_summary`) và `result_json` → persist trong `recap_plan_json` + `result_json`
cho UI / re-edit về sau.

**Env mới**: `RECAP_TWO_PASS` (1) · `GEMINI_STORY_THINKING_BUDGET` (8192) /
`GEMINI_STORY_MAX_TOKENS` / `GEMINI_STORY_TEMPERATURE` · `OPENAI_STORY_*` +
`OPENAI_STORY_REASONING_EFFORT` ("" = off) · `CLAUDE_STORY_*` +
`CLAUDE_STORY_THINKING_BUDGET` (0 = off) · `CLAUDE_RECAP_CACHE` (1).
Ngoài ra `GEMINI_RECAP_THINKING_BUDGET` mặc định đổi `0 → 8192` (recap pass-2 cũng suy luận).

**Chi phí**: +1 call/render (~$0.02, +~30s) — vô hình so với render 20–60′; caching
kéo token pass-2 xuống. **Test**: +10 (1900 passed, 0 regression). Commit `63b31b85`.

> Contract giữ nguyên: #1 result_json keys (`story_model`/`story_summary` additive),
> #2 không thêm field `RenderRequest`, #3 AI return None never raise. Frozen
> stages/routes/WS shape không đổi.
