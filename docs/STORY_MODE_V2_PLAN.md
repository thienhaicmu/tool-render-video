...
| F0 | ✅ DONE | 2026-07-10 | Tầng BASE mode-agnostic `components/studio/`: `StudioScreen`·`StudioCard`·`StudioField`·`StudioStepper`·`SegRow`(generic)·`RatioPicker`(generic, default 16:9) + `index.ts` barrel + `studio.css` (class `.st-*`, CHỈ token `styles/tokens.css`: `--bg-*`,`--border*`,`--text-1/2/3`,`--space-*`,`--radius-*`,`--ok/--fail/--accent`,`--brand-gradient`). `tsc -b` sạch (exit 0); guard: 0 import content-studio. Cả 2 studio kế thừa; content-studio migrate sau (không chặn). |
