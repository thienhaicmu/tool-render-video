...
| Verify | ✅ PASS | 2026-07-10 | Runtime real (key .env, ffmpeg) cả A&B — cap `STORY_MAX_IMAGES=3`/tier low. **A (paste):** completed · super=**1** image=**1** tts=5(=beat) · 2 char/1 visual/5 beat · 23.7s · visual_assets 1/1 file OK · `/story-visual`→FileResponse image/png · mp4 OK. **B (idea):** completed · super=**1** image=**2** tts=4(=beat) · 22.5s · 2/2 file OK · endpoint OK · mp4 OK. **Kết luận:** planning phẳng (1 super A&B) ✅, ảnh capped ≤ceiling ✅ (A: 1 wide visual cho 5 beat — đúng thiết kế camera-pan), V1/V2/V3 data OK ✅. **⚠️ "~1 TTS" thực tế = per-beat** (đúng deviation B6; cùng $ vì char-priced). Job rows + file verify đã dọn. |
| V3 | ✅ DONE | 2026-07-10 | FE grid ảnh thật — `StoryMonitor`: polling `fetchJobStoryPlan` **giữ refresh đến terminal** (trước dừng sau plan đầu → không thấy ảnh mới); **Visuals grid** (`<img>` từ `/api/jobs/{id}/story-visual/{vid}` khi `visual_assets[vid]` xuất hiện, else spinner+badge màu; đếm done/total; `VisualThumb` onError→fallback badge nếu ảnh bị prune); "now rendering" badge dùng **thumb thật** khi có. CSS grid `st-vt*`. `tsc -b` + `vite build` exit 0; guard OK. |
| V2 | ✅ DONE | 2026-07-10 | Persist ảnh tăng dần — `story_pipeline_v2._generate_images`: ảnh viết vào **thư mục bền `CACHE_DIR/story_visuals/{job_id}`** (không bị `rmtree` finalize, trong safe-root V1 → xem được cả trong lẫn sau render; cache prune tự thu hồi); **sau MỖI ảnh** `update_story_plan` + emit `story.visual.ready` (best-effort) → FE reveal từng cái. Baseline **2797→2799** (0 regression); 2 focused (persist n lần, out_dir bền, event/visual_id/total, fallback skip persist) + 4 e2e (patch `sp2.CACHE_DIR`). |
| V1 | ✅ DONE | 2026-07-10 | Endpoint ảnh mid-render — `routes/jobs.py` `GET /{job_id}/story-visual/{visual_id}` (MEDIUM). Path lấy từ plan persist `render.visual_assets` (server-gen, KHÔNG ghép từ URL); `visual_id` regex `^[A-Za-z0-9_-]{1,64}$`; resolve phải nằm trong `[TEMP_DIR, CACHE_DIR]` (endpoint unauth) → `FileResponse(image/png)`; 404 job/visual/file thiếu, 403 ngoài root. Baseline **2791→2797** (0 regression); 6 test (404 missing/bad-id/not-in-plan/file-missing, 200 served, **403 outside-roots**). |
| F4 | ✅ DONE | 2026-07-10 | Live UX. `StoryDirectorConsole.tsx` (overlay planning khi `planStory` chạy: đi bước THẬT theo dependency Đọc→Nhân vật→Bối cảnh→Key-visual→Timeline, mode B thêm "Sáng tác truyện"; timer 1.2s giữ bước cuối, honest không %, wire khi `busy && phase==='input'`). `StoryMonitor.tsx` (thật, bám `useRenderSocket` + polling `fetchJobStoryPlan`): stage rail (Phân tích→Dựng hình→Lời kể→Render→Xong theo stage+pct) + progress bar + total_sec, "now rendering" cue card (badge màu + narration + sub-step 🖼/🎙/🎬), cue strip theo part status, activity feed (liveEvents), badge partial/fail, video player khi DONE. CSS console+monitor. `tsc -b` exit 0; **`vite build` exit 0** (StoryStudio bundle 27.7kB); guard OK. Ghi chú: build rebuild `static-v2` (UI served) → tách commit `build(ui)`. |
