...
| **C.1 Phase 2** | [`c4f7d67`](#) | **`render_pipeline.py` wire-in** — reads `payload.use_story_intelligence`, calls `run_comprehension(...)` before the LLM call (cache-miss branch only). Double-gated by the env var + payload flag. Sacred Contract #3 try/except. NO provider touch yet. StoryModel is produced + persisted; Phase 3 wires consumer. | 0 new (zero regression on 2163 baseline; sentinel B6 flipped to assert-present) |
| **C.1 Phase 3** | [`44c0069`](#) | **Closure — providers + prompt + producer↔consumer wire** — `story_model` kwarg added to dispatcher + 3 providers + `build_render_plan_prompt`. New `_story_block_clips` helper injects whole-source semantic understanding into clips prompt. `render_pipeline.py` captures + forwards `_clip_story_model`. **PROMPT_VERSION NOT bumped** — empty story block renders to `""` so legacy prompts are byte-identical (LLM cache key SHA stable). **#1 strategic gap CLOSED.** | +6 (2163 → 2169 baseline; B7 sentinel flipped, prompt-format tests added) |
| **E2E wire smokes (D-2-motion + C.1)** | (this commit) | **Mock-based end-to-end validation** — 8 C.1 tests (dispatcher forwards story_model; all 3 providers thread; prompt contains STORY INTELLIGENCE block; full Comprehension→provider e2e via mocks; Sacred Contract #2 byte-identical). 9 D-2-motion tests (env default pinned at `"1"`; slice shape matches pixel-diff; kwarg threading clip_renderer↔motion/crop; part_render_encode load site; Policy A fallback chains; SceneMap DB round-trip; dispatcher iteration consumability). Catches wire breakage that unit tests miss. | +17 (2169 → 2186 baseline) |
...
| **C.1 Phase 3** (providers + prompt + producer↔consumer wire — **#1 strategic gap CLOSED**) | **C.1 Phase 3** |

### ⏳ Deferred (consumer-wiring follow-ups)

| Item | Priority | Risk | Effort | Notes |
|------|----------|------|--------|-------|
| **D-2-motion production-validate** — run 2-3 real-content renders; flip back to `0` if visual quality regresses on dissolves / lighting-change-in-shot content | Operator side | LOW | ~30min | Wire validated by 9 mock smoke tests; real-render quality validation still needs human eyes. If renders look bad: `export MOTION_USE_SCENE_MAP=0` |
| **C.1 production-validate** — flip a Clip job's `use_story_intelligence` to `true`, inspect the produced clips for theme/conflict grounding. Flip default to `true` once validated. | Operator side | LOW | ~30min | Wire validated by 8 mock smoke tests; real-render quality validation needs human eyes on clip selection. |
