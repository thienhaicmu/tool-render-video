...
- Static frontend: `backend/static/index.html` is the main shell. CSS is in `backend/static/css/app.css`. JS modules under `backend/static/js/` manage source setup, editor flow, render payloads, WebSocket/polling progress, output cards, history, download/upload UI, and navigation. Partials live in `backend/static/partials/`.
...
- `backend/static/css/app.css`: Shared UI styling. Avoid broad layout rewrites. Check render/editor/output/history views after changes.
