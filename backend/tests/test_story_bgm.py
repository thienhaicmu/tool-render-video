"""Story Mode v2 — per-scene background music (Q1).

Covers: Visual.bgm_mood (de)serialise + coercion, StoryPlan.bgm_scenes() grouping,
mixer.build_scene_bgm_track None-paths (no ffmpeg needed), and the super-prompt
carrying the bgm_mood field/vocab.
"""
...
def _plan() -> StoryPlan:
    p = StoryPlan(
        ...
        visuals=[
            Visual(id="v1", prompt="cold hall wide", bgm_mood="tense"),
            Visual(id="v2", prompt="sunrise field", bgm_mood="hopeful"),
        ],
        timeline=[
            Beat(id="b1", narration="Đêm lạnh buông.", visual_id="v1"),
            Beat(id="b2", narration="Hàn Phong bước tới.", visual_id="v1"),
            Beat(id="b3", narration="Bình minh lên.", visual_id="v2"),
        ],
    )
...
# ── domain: bgm_mood field ────────────────────────────────────────────────────

def test_visual_bgm_mood_roundtrip():
    p = _plan()
    p2 = StoryPlan.from_json(p.to_json())
    assert p2 is not None
    assert p2.visual("v1").bgm_mood == "tense"
    assert p2.visual("v2").bgm_mood == "hopeful"


def test_visual_bgm_mood_unknown_coerced_empty():
    p = StoryPlan.from_json(json.dumps({
        "timeline": [{"id": "b1", "narration": "hi", "visual_id": "v1"}],
        "visuals": [{"id": "v1", "prompt": "x", "bgm_mood": "banana"}],
    }))
    assert p is not None
    assert p.visual("v1").bgm_mood == ""   # unknown → "" (falls back to default folder)


# ── domain: bgm_scenes() grouping ─────────────────────────────────────────────

def test_bgm_scenes_group_consecutive_same_visual():
    scenes = _plan().bgm_scenes()
    # b1+b2 share v1 → ONE scene (tense); b3 v2 → hopeful. Two scenes total.
    assert len(scenes) == 2
    assert scenes[0][0] == "tense"
    assert scenes[1][0] == "hopeful"
    # windows are ordered, non-overlapping enough, start clamped >= 0
...
def test_bgm_scenes_empty_when_no_cues():
    p = StoryPlan(visuals=[Visual(id="v1", prompt="x", bgm_mood="epic")])
    assert p.bgm_scenes() == []


def test_bgm_scenes_missing_mood_is_empty_string():
    p = StoryPlan(
        visuals=[Visual(id="v1", prompt="x")],  # no bgm_mood
...
# ── prompt: bgm_mood carried into the super-prompt ────────────────────────────
...
def test_super_prompt_carries_bgm_mood_vocab():
    ...
    assert SUPER_PROMPT_VERSION == "s2"
