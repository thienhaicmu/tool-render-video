"""
camera_knowledge_retriever.py — Phase 53C camera knowledge retrieval.

Retrieves curated camera/framing/motion knowledge from the local knowledge registry.
Provides camera AI systems with expertise to improve reasoning, scoring,
recommendation, and quality evaluation.
...
Public API:
    ...
    build_camera_reasoning(pack, creator_style, motion_style)
        -> List[str]

Safety contract:
  - Local knowledge only: no internet, no subprocess, no network
  - Never mutates motion_crop, tracking, scene detection, or FFmpeg
...
logger = logging.getLogger("app.ai.knowledge.camera_retriever")
...
def retrieve_knowledge(
    ...
    """Retrieve camera knowledge items by domain and tags.

    Args:
        domain:        Knowledge category to retrieve. Default "camera".
    ...
    Returns:
        AICameraKnowledgePack — always non-None, never raises.
    ...
    try:
        ...
        logger.debug("camera_knowledge_retrieval_error: %s", exc)
        return AICameraKnowledgePack(
            available=False,
            domain=str(domain or "camera"),
            warnings=[f"retrieval_error:{type(exc).__name__}"],
        )


def build_camera_reasoning(
    pack: AICameraKnowledgePack,
    creator_style: Optional[str] = None,
    motion_style: Optional[str] = None,
) -> List[str]:
    """Build reasoning hint strings from a camera knowledge pack.

    Combines pack reasoning_hints with optional creator/motion style context.
    Returns human-readable notes for camera preference reasoning.

    Never raises. Metadata-only — does not mutate any camera execution.
    """
    try:
        ...
        if creator_style and motion_style:
            for item in pack.items:
                if item.creator_style and item.creator_style == creator_style:
                    hints.append(
                        f"{motion_style} style aligned with {item.title.lower()} guidance."
                    )
                    break
...
def _retrieve(
    ...
    domain = str(domain or "camera").strip() or "camera"
    ...
    if not all_items:
        logger.debug("camera_knowledge_empty domain=%s", domain)
        return AICameraKnowledgePack(
            available=False,
            domain=domain,
            warnings=["no_camera_knowledge_available"],
        )
    ...
    if not matched:
        logger.debug(
            "camera_knowledge_no_match domain=%s tags=%s", domain, requested_tags
        )
        return AICameraKnowledgePack(
            available=False,
            domain=domain,
            matched_tags=requested_tags,
            warnings=["no_matching_camera_knowledge"],
        )
    ...
    knowledge_items = [
        AICameraKnowledgeItem(
            ...
            tags=list(item.tags),
            camera_patterns=dict(item.camera_patterns),
    ...
    logger.debug(
        "camera_knowledge_retrieved domain=%s tags=%s items=%d",
...
def _build_hints(items: List[AICameraKnowledgeItem]) -> List[str]:
