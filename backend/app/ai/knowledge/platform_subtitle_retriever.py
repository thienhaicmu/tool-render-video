"""
platform_subtitle_retriever.py — Phase 55B Platform Subtitle Intelligence.

Retrieves subtitle-specific platform and creator-archetype knowledge from
knowledge/platforms/ and builds advisory platform_subtitle_context metadata.

Wraps the Phase 55A platform knowledge loader; filters to items that carry
subtitle domain guidance, then merges and ranks the result.
...
Public API:
    ...
    build_platform_subtitle_context(platform, creator_type, base_path)
        -> dict  ({"platform_subtitle_context": {...}})

Safety contract:
  - Local knowledge only: no internet, no subprocess, no cloud API
  - Never mutates subtitles, timing, segmentation, ASS, or FFmpeg
...
logger = logging.getLogger("app.ai.knowledge.platform_subtitle_retriever")

_MAX_RESULTS_BOUND = 10
_MIN_RESULTS_BOUND = 1
_MAX_REASONING_LINES = 5

# Guard: subtitle guidance keys that are safe to surface in metadata
_SAFE_GUIDANCE_KEYS = frozenset({
    "density_bias", "readability_priority", "keyword_emphasis",
    "line_count_preference", "max_words_per_line", "overload_risk_sensitivity",
    "mobile_safe_required", "animation_level", "style_preference",
    "avoid_dense_blocks", "concept_highlighting",
})
...
def retrieve_platform_subtitle_knowledge(
    platform: str = "",
    creator_type: str = "",
    ...
    """Retrieve subtitle-domain platform knowledge by platform and/or creator_type.

    Args:
        platform:     Platform identifier ("tiktok", "youtube_shorts", etc.)
        creator_type: Creator archetype ("podcast", "educational", etc.)
        tags:         Optional subtitle-specific tag filter (any-match).
    ...
    try:
        ...
        logger.debug("platform_subtitle_retrieval_error: %s", exc)
        return _fallback_retrieval(platform, creator_type)


def build_platform_subtitle_context(
    platform: str = "",
    creator_type: str = "",
    base_path: Any = None,
) -> dict:
    """Build platform_subtitle_context metadata dict for the edit plan. Never raises.

    Returns {"platform_subtitle_context": {...}} always.
    Fallback returns available=False context when no matching knowledge exists.

    Advisory only — context is metadata, never alters subtitle execution.
    """
    try:
        return _build_context(platform, creator_type, base_path)
    except Exception as exc:
        logger.debug("platform_subtitle_context_build_error: %s", exc)
        return {"platform_subtitle_context": _fallback_ctx(platform, creator_type)}
...
def _retrieve(
    ...
    # Filter 1: subtitle domain
    subtitle_items = [i for i in all_items if "subtitle" in (i.domains or [])]
    if not subtitle_items:
        return _fallback_retrieval(plat, ctype)

    # Filter 2: platform and/or creator_type
    filtered = _filter_by_platform_and_type(subtitle_items, plat, ctype)
    if not filtered:
        return _fallback_retrieval(plat, ctype)
...
def _build_context(
    ...
    # Merge subtitle guidance: later items set defaults, first item wins on conflicts
    merged: Dict[str, Any] = {}
    for m in reversed(matches):
        subtitle_g = (m.get("guidance") or {}).get("subtitle") or {}
        merged.update(subtitle_g)
    merged = _safe_guidance(merged)

    logger.debug(
        "platform_subtitle_context_built platform=%s creator_type=%s confidence=%.3f",
...
def _safe_guidance(guidance: dict) -> dict:
    """Return only safe subtitle guidance keys, strip any forbidden content."""
...
def _build_reasoning(
    ...
    lines: List[str] = []
    for item in items:
        sub_guidance = (item.guidance or {}).get("subtitle", {})
        density = sub_guidance.get("density_bias", "")
        readability = sub_guidance.get("readability_priority", "")

        if item.platform and item.platform != "general" and item.creator_type:
            if density and readability:
                lines.append(
                    f"{item.title} supports {density} density with {readability} readability priority"
                )
            else:
                lines.append(f"{item.title} provides subtitle guidance for {item.platform} {item.creator_type}")
        elif item.platform and item.platform != "general":
            if density:
                lines.append(f"{item.title} favors {density} subtitle density for {item.platform}")
            else:
                lines.append(f"{item.title} provides subtitle guidance for {item.platform}")
        elif item.creator_type:
            if density:
                lines.append(f"{item.title} supports {density} subtitle density for {item.creator_type} creators")
            else:
                lines.append(f"{item.title} provides subtitle guidance for {item.creator_type} creators")
        else:
            lines.append(f"{item.title} provides platform subtitle guidance")
...
def _fallback_retrieval(platform: str, creator_type: str) -> dict:
    return {
        ...
        "confidence": 0.0,
        "reasoning": [],
        "warnings": ["no_platform_subtitle_knowledge"],
    }
