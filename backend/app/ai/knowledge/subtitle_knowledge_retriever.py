"""
subtitle_knowledge_retriever.py — Phase 53B subtitle knowledge retrieval.

Retrieves curated subtitle knowledge from the local knowledge registry.
Provides subtitle AI systems with curated knowledge to improve reasoning,
scoring, recommendation, and quality evaluation.

Supports:
  - subtitle preference reasoning (Phase 50A)
  - subtitle safe influence (Phase 50C)
...
Public API:
    ...
    build_subtitle_reasoning(pack, creator_style, subtitle_style)
        -> List[str]

Safety contract:
  - Local knowledge only: no internet, no subprocess, no network
  - Never mutates subtitle execution, timing, or segmentation
...
logger = logging.getLogger("app.ai.knowledge.subtitle_retriever")
...
def retrieve_knowledge(
    ...
    """Retrieve subtitle knowledge items by domain and tags.

    Args:
        domain:        Knowledge category to retrieve. Default "subtitle".
    ...
    Returns:
        AISubtitleKnowledgePack — always non-None, never raises.
    ...
    try:
        ...
        logger.debug("subtitle_knowledge_retrieval_error: %s", exc)
        return AISubtitleKnowledgePack(
            available=False,
            domain=str(domain or "subtitle"),
            warnings=[f"retrieval_error:{type(exc).__name__}"],
        )


def build_subtitle_reasoning(
    pack: AISubtitleKnowledgePack,
    creator_style: Optional[str] = None,
    subtitle_style: Optional[str] = None,
) -> List[str]:
    """Build reasoning hint strings from a subtitle knowledge pack.

    Combines pack reasoning_hints with optional creator/subtitle style context.
    Returns human-readable notes for subtitle preference reasoning.

    Never raises. Metadata-only — does not mutate any subtitle execution.
    """
    try:
        ...
        if creator_style and subtitle_style:
            for item in pack.items:
                if item.creator_style and item.creator_style == creator_style:
                    hints.append(
                        f"{subtitle_style} style aligned with {item.title.lower()} guidance."
                    )
                    break
...
def _retrieve(
    ...
    domain = str(domain or "subtitle").strip() or "subtitle"
    ...
    if not all_items:
        logger.debug("subtitle_knowledge_empty domain=%s", domain)
        return AISubtitleKnowledgePack(
            available=False,
            domain=domain,
            warnings=["no_subtitle_knowledge_available"],
        )
    ...
    if not matched:
        logger.debug(
            "subtitle_knowledge_no_match domain=%s tags=%s", domain, requested_tags
        )
        return AISubtitleKnowledgePack(
            available=False,
            domain=domain,
            matched_tags=requested_tags,
            warnings=["no_matching_subtitle_knowledge"],
        )
    ...
    knowledge_items = [
        AISubtitleKnowledgeItem(
            ...
            tags=list(item.tags),
            subtitle_patterns=dict(item.subtitle_patterns),
    ...
    logger.debug(
        "subtitle_knowledge_retrieved domain=%s tags=%s items=%d",
...
def _build_hints(items: List[AISubtitleKnowledgeItem]) -> List[str]:
