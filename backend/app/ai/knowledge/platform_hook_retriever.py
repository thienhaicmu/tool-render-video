"""
platform_hook_retriever.py — Phase 55D Platform Hook & Retention Intelligence.

Retrieves hook- and retention-specific platform and creator-archetype knowledge
from knowledge/platforms/ and builds advisory platform_hook_context metadata.

Wraps the Phase 55A platform knowledge loader; filters to items that carry
hook domain guidance, then merges and ranks the result.
...
Public API:
    ...
    build_platform_hook_context(platform, creator_type, base_path)
        -> dict  ({"platform_hook_context": {...}})
...
logger = logging.getLogger("app.ai.knowledge.platform_hook_retriever")

_MAX_RESULTS_BOUND = 10
_MIN_RESULTS_BOUND = 1
_MAX_REASONING_LINES = 5

# Guard: hook guidance keys that are safe to surface in metadata
_SAFE_GUIDANCE_KEYS = frozenset({
    "first_3s_priority", "retention_priority", "curiosity_strength",
    "hook_energy", "slow_intro_risk", "payoff_expectation",
    "hook_style", "open_loop_quality", "first_5s_retention",
    "hype_level", "trust_priority", "clarity_priority",
    "emotional_stakes", "narrative_tension",
})
...
def retrieve_platform_hook_knowledge(
    platform: str = "",
    creator_type: str = "",
    ...
    """Retrieve hook-domain platform knowledge by platform and/or creator_type.

    Args:
        platform:     Platform identifier ("tiktok", "youtube_shorts", etc.)
        creator_type: Creator archetype ("podcast", "educational", "storytelling", etc.)
        tags:         Optional hook-specific tag filter (any-match).
    ...
    try:
        ...
        logger.debug("platform_hook_retrieval_error: %s", exc)
        return _fallback_retrieval(platform, creator_type)


def build_platform_hook_context(
    platform: str = "",
    creator_type: str = "",
    base_path: Any = None,
) -> dict:
    """Build platform_hook_context metadata dict for the edit plan. Never raises.

    Returns {"platform_hook_context": {...}} always.
    Fallback returns available=False context when no matching knowledge exists.

    Advisory only — context is metadata, never alters hook execution.
    """
    try:
        return _build_context(platform, creator_type, base_path)
    except Exception as exc:
        logger.debug("platform_hook_context_build_error: %s", exc)
        return {"platform_hook_context": _fallback_ctx(platform, creator_type)}
...
def _retrieve(
    ...
    # Filter 1: hook domain
    hook_items = [i for i in all_items if "hook" in (i.domains or [])]
    if not hook_items:
        return _fallback_retrieval(plat, ctype)

    # Filter 2: platform and/or creator_type
    filtered = _filter_by_platform_and_type(hook_items, plat, ctype)
    if not filtered:
        return _fallback_retrieval(plat, ctype)
...
def _build_context(
    ...
    # Merge hook guidance: later items set defaults, first item wins on conflicts
    merged: Dict[str, Any] = {}
    for m in reversed(matches):
        hook_g = (m.get("guidance") or {}).get("hook") or {}
        merged.update(hook_g)
    merged = _safe_guidance(merged)

    logger.debug(
        "platform_hook_context_built platform=%s creator_type=%s confidence=%.3f",
...
def _safe_guidance(guidance: dict) -> dict:
    """Return only safe hook guidance keys, strip any forbidden content."""
...
def _build_reasoning(
    ...
    lines: List[str] = []
    for item in items:
        hook_guidance = (item.guidance or {}).get("hook", {})
        first_3s = hook_guidance.get("first_3s_priority", "")
        retention = hook_guidance.get("retention_priority", "")
        hook_style = hook_guidance.get("hook_style", "")

        if item.platform and item.platform != "general" and item.creator_type:
            if first_3s and retention:
                lines.append(
                    f"{item.title} prioritizes {first_3s} first-3-second attention "
                    f"with {retention} retention priority"
                )
            elif hook_style:
                lines.append(f"{item.title} recommends {hook_style} hook style for {item.platform}")
            else:
                lines.append(f"{item.title} provides hook guidance for {item.platform} {item.creator_type}")
        elif item.platform and item.platform != "general":
            if first_3s:
                lines.append(f"{item.title} supports {first_3s} first-3-second hook strength for {item.platform}")
            else:
                lines.append(f"{item.title} provides hook guidance for {item.platform}")
        elif item.creator_type:
            if hook_style:
                lines.append(
                    f"{item.title} recommends {hook_style} hook style for {item.creator_type} creators"
                )
            elif retention:
                lines.append(
                    f"{item.title} supports {retention} retention priority for {item.creator_type} creators"
                )
            else:
                lines.append(f"{item.title} provides hook guidance for {item.creator_type} creators")
        else:
            lines.append(f"{item.title} provides platform hook and retention guidance")
...
def _fallback_retrieval(platform: str, creator_type: str) -> dict:
    return {
        ...
        "confidence": 0.0,
        "reasoning": [],
        "warnings": ["no_platform_hook_knowledge"],
    }
