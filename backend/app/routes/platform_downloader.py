...
def _validate_download_url(url: str) -> str:
    ...
    if not is_allowed_url(url):
        raise HTTPException(
            status_code=400,
            detail=f"Domain '{host}' is not supported. Supported: YouTube, TikTok, Instagram, Facebook, X, Bilibili, Reddit, Vimeo, Dailymotion.",
        )
    return url
...
class DownloadStartRequest(BaseModel):
    url: str
    output_dir: str
    quality: str = "best"   # best | 1080p | 720p | 480p (reserved for future)
...
def cancel_job(job_id: str):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Download job not found")
    # Mark cancelled if still running (executor will handle gracefully on next DB read)
