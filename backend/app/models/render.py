...
class RenderRequest(BaseModel):
    ...
    def _validate_content_visual_provider(cls, v) -> str:
        # v1 ships only the "local" provider. Unknown/future values coerce to
        # "local" so a payload authored for a not-yet-shipped provider still
        # renders offline instead of failing (Sacred Contract #2).
        v = str(v or "local").strip().lower()
        return v if v in {"local"} else "local"
