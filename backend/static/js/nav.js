function setView(view){
  ...
  // Pipeline: add inPipeline when editor opens; preserve through render view;
  // clear only when leaving the render/editor workflow entirely.
  if (mainArea) {
    if (isEditor) mainArea.classList.add('inPipeline');
    else if (!isRender) mainArea.classList.remove('inPipeline');
  }
  ...
  const wsView = qs('view_workspace');
  if (wsView) wsView.classList.toggle('hiddenView', !isWorkspace);
  ...
  const showMainContent = !isRender && !isEditor && !isDownload && !isHistory && !isReview && !isWorkspace;
  ...
  if (isWorkspace && typeof CreatorWorkspace  !== 'undefined') { CreatorWorkspace.init(); }
  ...
  document.body.classList.toggle('is-workspace-active', isWorkspace);
...
// ── Workflow step strip ────────────────────────────────────────────────────────
// updateWfStrip: reads real app state (currentJobId, panel visibility) to
// determine which workflow step is active. Called by setView() and render-ui.js
// hooks so the strip always reflects actual pipeline position.
function updateWfStrip() {
  ...
  var step;
  if      (outputVis && !completionVis) step = 'export';
  else if (completionVis || outputVis)  step = 'review';
  else if (hasActiveJob)                step = 'render';
  else                                  step = 'source';

  _setWfStep(strip, step);
}

function _setWfStep(strip, activeStep) {
  var order = ['source', 'prepare', 'render', 'review', 'export'];
  var activeIdx = order.indexOf(activeStep);
  strip.querySelectorAll('.wfCard[data-wf-step]').forEach(function(card) {
    var step = card.dataset.wfStep;
    var idx  = order.indexOf(step);
    card.classList.remove('is-active', 'is-done');
    if (!activeStep) return;
    if (step === activeStep)  card.classList.add('is-active');
    else if (idx < activeIdx) card.classList.add('is-done');
  });
}
