...
/** Per-beat estimated seconds — mirror backend StoryPlan.beat_est_sec. */
export function beatEstSec(b: Beat, language: string): number {
  const n = (b.narration || '').trim().length
  if (!n) return Math.max(0, b.hold_sec || 0)
  const cps = CPS[(language || 'vi').slice(0, 2)] ?? 14
  return n / cps / (b.reading_speed || 1)
}
