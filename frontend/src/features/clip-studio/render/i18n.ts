...
const T = {
  EN: {
    ...
    cfgReactionMode: 'Reaction mode',
    cfgReactionModeHint: 'Faceless reaction: AI leads in, holds a freeze-frame for suspense, then lets the original audio land the moment',
    cfgReactionIntensity: 'Reaction density',
    cfgVoiceSrcRecapNote: 'Recap writes its own narration — the AI authors the voice-over for every scene. Source / Reaction options don’t apply here.',
    cfgRenderMode: 'RENDER MODE',
    cfgModeClips: 'Short clips',
    cfgModeClipsDesc: 'Several short vertical clips',
    cfgModeRecap: 'Recap / Review',
    cfgModeRecapDesc: 'One long video — AI picks scenes + acts, narrates the whole film',
    cfgStoryIntel: 'Story Intelligence',
    cfgStoryIntelDesc: 'AI reads the whole film first, then grounds clip picks in its story understanding',
    ...
    // P1.2 — previously hardcoded EN in StepConfigure
    cfgOutputCount: 'Output videos',
    cfgFocus: 'Focus',
    cfgQuality: 'Quality',
    cfgAiProvider: 'AI Provider',
    cfgAiAutoSelect: 'Auto-select clips with AI',
    cfgAiAutoSelectDesc: 'AI reads the transcript and picks the strongest moments automatically.',
    cfgAiKeyNote: '✦ Uses the API key configured on this machine — click Test to verify the connection.',
    cfgModeQuick: 'Quick', cfgModeAdvanced: 'Advanced',
    cfgBrowse: 'Browse',
    cfgDefaultDir: (d: string) => `Default: ${d}`,
    ...
    // P1.2 — recap strings (were hardcoded VI inside the EN UI)
    rndRecapProgress: (done: number, total: number, ep: number) =>
      `${done} / ${total} scenes${ep > 0 ? ` · ${ep} episode${ep !== 1 ? 's' : ''}` : ''}`,
    rndRecapBuilding: (scenes: number, ep: number) =>
      `Building ${scenes} scenes → ${ep} episode${ep !== 1 ? 's' : ''} — recap in progress`,
    rndRecapDone: (ep: number) => `Recap complete — ${ep} episode${ep !== 1 ? 's' : ''}`,
    rndRecapMeta: (scenes: number, ep: number, ratio: string) =>
      `${scenes} scenes · ${ep} episode${ep !== 1 ? 's' : ''} · ${ratio}`,
    rndRecapPlanning: '🎬 RECAP — building the scene plan…',
    rndRecapAssembling: '✓ SCENES DONE · ASSEMBLING EPISODES',
    rndRecapBuildingLbl: '● BUILDING SCENES',
    rndRecapEpisodeN: (n: number) => `Episode ${n}`,
    rndRecapSceneN: (n: number) => `Scene ${n}`,
    rndRecapScenesShort: (d: number, tot: number) => `${d}/${tot} scenes`,
    rndRecapOriginalAudio: '🔊 original audio',
    rndRecapNarration: '🎙 narration',
    rndRecapOriginalLine: '🔊 Letting the film’s own audio carry this scene',
    rndRecapNarrWriting: 'writing narration…',
    rndRecapNarrWaiting: 'awaiting narration',
    rndRecapClimax: 'climax',
    // Results
    ...
    // P1.2 — previously hardcoded EN in StepResults
    resComplete: 'Render Complete',
    resFailedTitle: 'Render Failed',
    resReadyToPublish: (n: number) => `${n} clip${n !== 1 ? 's' : ''} ready to publish`,
    resOpenFolder: 'Open Folder',
    resTopScore: 'Top Score', resKpiClips: 'Clips', resKpiTotal: 'Total',
    resAiQualityScore: 'AI Quality Score',
    resAiAnalysis: 'AI Analysis',
    resClipRanking: 'Clip Ranking',
    resMetrics: 'Metrics',
    resAllOk: '✓ No issues — all quality checks passed',
    resQualityLoading: 'Quality data loading…',
    ...
    resWhyTop: 'Strongest signal', resWhyDown: 'Held back by',
    resSigViral: 'Viral', resSigHook: 'Hook', resSigRetention: 'Retention',
    resSigSpeech: 'Speech', resSigMarket: 'Market fit', resSigDuration: 'Length fit',
    resScoreEduTitle: 'How scores work',
    resScoreEduBody: 'Each clip gets a 0-100 score from the AI Director, combining hook strength, viral signals, retention, speech density and length fit. 85+ is viral-ready, 70+ high impact, 55+ good. Your thumbs up/down teaches the AI what works for your channel.',
    rndAiAnalyzing: 'AI Director is reading your video - picking the strongest moments...',
    rndAiTranscribing: 'Transcribing the audio with Whisper AI...',
    rndAiRendering: (n: number) => `Rendering ${n} clip${n !== 1 ? 's' : ''} - AI keeps the subject in frame`,
    rndAiDone: (n: number) => `Analysis complete - ${n} clip${n !== 1 ? 's' : ''} selected`,
    rndProcessing: 'Processing...',
    actCutting: 'Extracting video segment',
    actTranscribing: 'Generating subtitles',
    actRendering: 'Encoding clip',
  ...
  VI: {
    ...
    cfgReactionMode: 'Chế độ reaction',
    cfgReactionModeHint: 'Reaction giấu mặt: AI dẫn trước, đứng hình câu giờ, rồi để tiếng gốc bật lên ở cao trào',
    cfgReactionIntensity: 'Mật độ reaction',
    cfgVoiceSrcRecapNote: 'Recap tự viết lời thuyết minh — AI soạn voice-over cho từng cảnh. Tùy chọn Nguồn / Reaction không áp dụng ở đây.',
    cfgRenderMode: 'CHẾ ĐỘ DỰNG',
    cfgModeClips: 'Clip ngắn',
    cfgModeClipsDesc: 'Nhiều clip dọc ngắn',
    cfgModeRecap: 'Recap / Review',
    cfgModeRecapDesc: 'Một video dài — AI chọn cảnh + chia chương, thuyết minh cả phim',
    cfgStoryIntel: 'Hiểu toàn phim',
    cfgStoryIntelDesc: 'AI đọc hiểu cả phim trước, rồi chọn clip dựa trên mạch truyện tổng thể',
    ...
    // P1.2 — chuỗi từng hardcode EN trong StepConfigure
    cfgOutputCount: 'Số video xuất',
    cfgFocus: 'Tiêu điểm',
    cfgQuality: 'Chất lượng',
    cfgAiProvider: 'AI Provider',
    cfgAiAutoSelect: 'AI tự chọn clip',
    cfgAiAutoSelectDesc: 'AI đọc transcript và tự chọn những khoảnh khắc đắt giá nhất.',
    cfgAiKeyNote: '✦ Dùng API key đã cấu hình trên máy này — bấm Test để kiểm tra kết nối.',
    cfgModeQuick: 'Nhanh', cfgModeAdvanced: 'Nâng cao',
    cfgBrowse: 'Chọn…',
    cfgDefaultDir: (d: string) => `Mặc định: ${d}`,
    ...
    // P1.2 — chuỗi recap (trước đây hardcode VI lẫn trong UI EN)
    rndRecapProgress: (done: number, total: number, ep: number) =>
      `${done} / ${total} cảnh${ep > 0 ? ` · ${ep} tập` : ''}`,
    rndRecapBuilding: (scenes: number, ep: number) =>
      `Dựng ${scenes} cảnh → ${ep} tập — recap đang chạy`,
    rndRecapDone: (ep: number) => `Recap xong — ${ep} tập`,
    rndRecapMeta: (scenes: number, ep: number, ratio: string) =>
      `${scenes} cảnh · ${ep} tập · ${ratio}`,
    rndRecapPlanning: '🎬 RECAP — đang dựng kế hoạch cảnh…',
    rndRecapAssembling: '✓ ĐÃ DỰNG XONG · ĐANG GHÉP TẬP',
    rndRecapBuildingLbl: '● ĐANG DỰNG',
    rndRecapEpisodeN: (n: number) => `Tập ${n}`,
    rndRecapSceneN: (n: number) => `Cảnh ${n}`,
    rndRecapScenesShort: (d: number, tot: number) => `${d}/${tot} cảnh`,
    rndRecapOriginalAudio: '🔊 tiếng gốc',
    rndRecapNarration: '🎙 thuyết minh',
    rndRecapOriginalLine: '🔊 Để tiếng gốc của phim tự nói',
    rndRecapNarrWriting: 'đang dựng lời thuyết minh…',
    rndRecapNarrWaiting: 'chờ thuyết minh',
    rndRecapClimax: 'cao trào',
    ...
    // P1.2 — chuỗi từng hardcode EN trong StepResults
    resComplete: 'Render hoàn tất',
    resFailedTitle: 'Render thất bại',
    resReadyToPublish: (n: number) => `${n} clip sẵn sàng đăng`,
    resOpenFolder: 'Mở thư mục',
    resTopScore: 'Điểm cao nhất', resKpiClips: 'Clip', resKpiTotal: 'Tổng',
    resAiQualityScore: 'Điểm chất lượng AI',
    resAiAnalysis: 'Phân tích AI',
    resClipRanking: 'Xếp hạng clip',
    resMetrics: 'Chỉ số',
    resAllOk: '✓ Không có vấn đề — đạt mọi kiểm tra chất lượng',
    resQualityLoading: 'Đang tải dữ liệu chất lượng…',
    ...
    resWhyTop: 'Điểm mạnh nhất', resWhyDown: 'Bị trừ điểm vì',
    resSigViral: 'Viral', resSigHook: 'Hook', resSigRetention: 'Giữ chân',
    resSigSpeech: 'Mật độ thoại', resSigMarket: 'Hợp thị trường', resSigDuration: 'Độ dài phù hợp',
    resScoreEduTitle: 'Điểm số hoạt động thế nào',
    resScoreEduBody: 'Mỗi clip được AI Director chấm 0-100, kết hợp độ mạnh của hook, tín hiệu viral, khả năng giữ chân, mật độ thoại và độ dài phù hợp. 85+ là sẵn sàng viral, 70+ hiệu quả cao, 55+ tốt. Bấm 👍/👎 để dạy AI hiểu kênh của bạn.',
    rndAiAnalyzing: 'AI Director đang đọc video của bạn - chọn những khoảnh khắc đắt nhất...',
    rndAiTranscribing: 'Đang phiên âm audio bằng Whisper AI...',
    rndAiRendering: (n: number) => `Đang render ${n} clip - AI giữ chủ thể trong khung hình`,
    rndAiDone: (n: number) => `Phân tích xong - đã chọn ${n} clip`,
    rndProcessing: 'Đang xử lý...',
    actCutting: 'Cắt đoạn video',
    actTranscribing: 'Tạo phụ đề',
    actRendering: 'Encode clip',
